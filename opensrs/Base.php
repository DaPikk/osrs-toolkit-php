...
class Base
{
    ...
    private function init_socket()
    {
        ...
        $scheme = rtrim((string) CRYPT_TYPE, ':/');
        ...
        $this->_socket = @stream_socket_client(
            $scheme.'://'.OSRS_HOST.':'.OSRS_SSL_PORT,
    ...
    private function writeData(&$fh, $msg)
    {
        ...
        $this->writeAll($fh, $header);
        $this->writeAll($fh, $msg);
    }

    private function writeAll($fh, $data)
    {
        $offset = 0;
        $length = strlen($data);
        while ($offset < $length) {
            ...
            if ($written === false || $written === 0) {
                throw new Exception('oSRS Error - Unable to write the complete request.');
            }
            $offset += $written;
        }
    }
    ...
    private function readHeader($fh, $timeout = 5)
    {
        ...
        $line = fgets($fh, 4000);
        if ($line === false) {
            throw new Exception('oSRS Error - Empty response header.');
        }
        $http_log .= $line;
        ...
        while ($line != CRLF) {
            $line = fgets($fh, 4000);
            if ($line === false) {
                throw new Exception('oSRS Error - Unexpected end of response header.');
            }
            $http_log .= $line;
    ...
    private function readData($fh, $timeout = 5)
    {
        ...
        $contentLength = $header['content-length'] ?? null;
        if (!$header || !is_string($contentLength) || !preg_match('/^\d+$/', trim($contentLength))) {
            $this->close_socket();
        ...
        $len = (int) trim($contentLength);
        if ($len < 1 || $len > self::MAX_RESPONSE_BYTES) {
            $this->close_socket();
            throw new Exception('oSRS Error - Response exceeds the configured size limit.');
        }
        $line = '';
        while (strlen($line) < $len) {
            $chunk = fread($fh, min(8192, $len - strlen($line)));
